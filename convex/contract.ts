...
export const createSingleContractWithBoard = mutation({
    args: {
        boardId: v.id("board"),
        contract: v.object({
            epicId: v.optional(v.string()),
    ...
    handler: async (ctx, args) => {
        const contractId = await createContractHelper(ctx, args.contract);
        if (contractId !== null) {
            await createBoardToContractHelper(ctx, args.boardId, contractId);
        }
    },
});
