...
export default function MatchTimeManagement({
    ...
    useEffect(() => {
        ...
        countdownRef.current = setInterval(() => {
            if (match.startTime !== undefined) {
                const rawUpdatedTimeFromStart = match.startTime - Date.now();
                const absUpdatedTimeFromStart = Math.abs(
                    rawUpdatedTimeFromStart,
                );

                if (rawUpdatedTimeFromStart < 0) {
                    matchStatusMutation({
                        status: "active",
                        matchId: match._id,
                    });
                ...
                setTimeFromStart(absUpdatedTimeFromStart);
            }
        }, 100);
    ...
    return (
        <div className="w-full">
            <div>
                {timeFromStart !== undefined && match.startTime !== -1 && (
                    <p
                        data-matchactive={match.status === "active"}
                    ...
                    >
                        ...
                        >
                            <TimeString ms={timeFromStart} />
                        </span>
                    </p>
                )}
                {(match.startTime === -1 || match.startTime === undefined) && (
                    <p className="text-2xl font-bold">{"No match time set"}</p>
                )}
            </div>
            <div>
                <p>
                    ...
                    <span>
                        {match.startTime === undefined || match.startTime === -1
                            ? "Unset"
                            : new Date(match.startTime).toLocaleString()}
                    </span>
                </p>
                <p>
                    <span className="font-bold">{"Grace Period Length: "}</span>
                ...
                <div className="grid grid-cols-2 gap-2 max-w-96">
                    <div className="flex flex-col gap-1">
                        ...
                        >
                            ...
                            <option value="0.1" className="bg-slate-500">
                                5 minutes
                            </option>
                        ...
                        <button
                            className="mt-1 py-2 bg-slate-800 hover:underline"
                    ...
                    <div className="flex flex-col gap-1">
                        <label
                            data-matchstarted={match.status === "active"}
                            className="mr-[0.5ch] font-bold"
