...
export default function BingoBoard({
    ...
    function GetTopSubmission(contractId: Id<"contract">) {
        ...
        return submissions[topSubmissionIndex];
    }
    ...
    for (let i = 0; i < size ** 2; i++) {
        ...
        if (contract) {
            const topSubmission = GetTopSubmission(contract._id);
            const topTeamColor =
                topSubmission === undefined
                    ? ""
                    : teams.find((team) => team._id === topSubmission.teamId)
                          ?.color + "d9";

            bingoSquares.push(
                <div
                    key={i}
                    className="bingo-card grid aspect-square border-2 border-slate-600 hover:border-slate-200 cursor-default data-[selected=true]:scale-105 data-[selected=true]:border-slate-200 data-[selected=true]:z-20 data-[selected=true]:shadow-[0_0_5px_3px] shadow-slate-200 origin-center"
                ...
                >
                    ...
                    >
                        <div
                            className="grid content-center px-0.5 bg-slate-900/80 align-middle z-10"
                            style={{
                                backgroundColor: topTeamColor,
                            }}
                        >
